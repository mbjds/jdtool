...
class voucherInit
{
    public function __construct()
    {
        ...
        add_action('admin_head', [ $this, 'hide_row_action_admin' ]);
        ...
        add_action('manage_aerovouchers_posts_custom_column', function ($column_name, $post_id) {
            ...
            if ('actions' == $column_name) {
                $ii = 2;
                echo '<a href="' . get_permalink() . '"><i  data-id="' . $post_id . '" style="font-size: 22px; padding: 0 7px" class="rTrue editV  fa-regular  fa-pen-to-square"></i></a>';
                for ($i = 0; $i < $ii; ++ $i) {
                    switch ($i) {
                        case 0:
    ...
    public function hide_row_action_admin()
    {
        global $post_type;
        if ('aerovouchers' === $post_type) {
            ?>
          <style>
              .row-actions {
                  display: none;
